document.addEventListener('DOMContentLoaded', () => {
            ...
            while (loginAttempts < maxLoginAttempts) {
                try {
                    ...
                    if (loginAttempts >= maxLoginAttempts) {
                        alert(`Failed to login after ${maxLoginAttempts} attempts: ${error.message}`);
    ...
    sourceCode.addEventListener('click', () => {
        window.open('https://github.com/aullah01/Hamster-Key-Generator', '_blank');
    });

    const generateClientId = () => {
        const timestamp = Date.now();
        const randomNumbers = Array.from({ length: 19 }, () => Math.floor(Math.random() * 50)).join('');
